"""
January 2024 by K.E. Spaans - addition of T2-IVIM (note: code is not entirely finished)
...
UPDATED November 2023 by Kelly Spaans
- Code now includes the T2-IVIM model
...
def fit_dats(bvalues, TEs, dw_data, arg, savename=None):
    """
    ...
    arg.model --> string with the fit model; allowed: bi-exp, tri-exp, T2 (note only lsq and seg are implemented for tri-exp and T2)
    ...
    :param TEs: 1D Array of TEs used
    ...
    :return paramslsq: 2D array containing the fit parameters D, f (Fp), D* (Dp), (T2p, T2t), and, optionally, S0, for each voxel
    """
        ...
        if not arg.load_lsq:
            ...
            if arg.method == 'seg':
                if arg.model == 'bi-exp':
                    ...
                    print('running segmented T2-IVIM fit\n')
                    paramslsq = fit_segmented_array_T2_ivim(bvalues, TEs, dw_data, njobs=arg.jobs, bounds=arg.bounds)
                    # save results if parameter savename is given
                    if savename is not None:
                        np.savez(savename, paramslsq=paramslsq)
                ...
                if arg.model == 'bi-exp':
                    ...
                    # save results if parameter savename is given
                    if savename is not None:
                        np.save(savename, paramslsq=paramslsq)
                
                elif (arg.model == 'T2'):
                    print('running conventional T2-IVIM fit\n')
                    paramslsq = fit_least_squares_array_T2_ivim(bvalues, TEs, dw_data, S0_output=True, fitS0=arg.fitS0,
                                                                njobs=arg.jobs,
                                                                bounds=arg.bounds)
                    # save results if parameter savename is given
                    if savename is not None:
                ...
                if (arg.model == 'T2'):
                    raise Exception('the choice T2-IVIM bayes not implemented')

            else:
                raise Exception('the choice lsq-fit is not implemented. Try ''lsq'', ''seg'' or ''bayes''')
        else:
            # if we choose to load the fit
            print('loading fit\n')
...
def goodness_of_fit(bvalues, Dt, Fp, Dp, S0, dw_data, Fp2 = None, Dp2 = None, T2p=None, T2t=None, TEs=None):
    """
    Calculates the R-squared as a measure for goodness of fit.
    input parameters are
    :param b: 1D Array b-values
    :param TEs: 1D Array TEs
    ...
    """
    # flatten
    xdata = np.hstack((bvalues, TEs))

    # simulate the IVIM signal given the D, f, D* and S0
    try:
        if Fp2 is None and T2p is None:
        ...
        elif T2p is not None: 
            datasim = T2_ivim(np.tile(np.expand_dims(bvalues, axis=0), (len(Dt), 1)),
                           np.tile(np.expand_dims(S0*(1-Fp), axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(Dt, axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(Fp*S0, axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(Dp, axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(T2p, axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(T2t, axis=1), (1, len(bvalues))),
                           ).astype('f')
        ...
        if Fp2 is None and T2p is None:
            ...
            datasim = T2_ivim(xdata, S0 * (1 - Fp), Dt, Fp * S0, Dp, T2p, T2t)
        norm = np.mean(dw_data)
        ss_tot = np.sum(np.square(dw_data - norm))
        ss_res = np.sum(np.square(dw_data - datasim))
        R2 = 1 - (ss_res / ss_tot)  # R-squared
        if Fp2 is None and T2p is None: # TODO: working statement for now
            adjusted_R2 = 1-((1-R2)*(len(bvalues))/(len(bvalues)-4-1))
        else:
            adjusted_R2 = 1 - ((1 - R2) * (len(bvalues)) / (len(bvalues) - 6 - 1))
        # from matplotlib import pyplot as plt
...
def MSE(bvalues, Dt, Fp, Dp, S0, dw_data, T2p=None, T2t=None): # TODO: was not implemented for tri-exp, but implementated for T2-IVIM now
    """
...
def T2_ivimN(xdata, Fp0, Dt, Fp, Dp, T2p, T2t):
    # Extract the values of bvalues and TEs from the flattened input array x
    bvalues = xdata[:len(xdata) // 2]
    TEs = xdata[len(xdata) // 2:]
    # IVIM function in which we try to have equal variance in the different IVIM parameters; equal variance helps with certain fitting algorithms
    return ((Fp / 10) * np.exp(-bvalues * (Dp / 100)) * np.exp(-TEs/(T2p / 10)) + (Fp0 / 10) * np.exp(-bvalues * (Dt / 1000)) * np.exp(-TEs/(T2t / 10)))


def T2_ivimN_noS0(xdata, Dt, Fp, Dp, T2p, T2t):
    # Extract the values of bvalues and TEs from the flattened input array x
    bvalues = xdata[:len(xdata) // 2]
    TEs = xdata[len(xdata) // 2:]
    # IVIM function in which we try to have equal variance in the different IVIM parameters and S0=1
    return ((Fp / 10) * np.exp(-bvalues * (Dp / 100)) * np.exp(-TEs/(T2p / 10)) + (1 - (Fp / 10)) * np.exp(-bvalues * (Dt / 1000)) * np.exp(-TEs/(T2t / 10)))


def T2_ivim(bvalues, TEs, Fp0, Dt, Fp, Dp, T2p, T2t):
    # regular IVIM function
...
def fit_segmented_array_T2_ivim(bvalues, TEs, dw_data, njobs=4, bounds=([0, 0, 0, 0, 0, 0], [2.5, 0.003, 1, 0.05, 0.15, 0.12]), cutoff=75):
    """
    This is an implementation of the segmented fit, in which we first estimate D using a curve fit to b-values>cutoff;
    then estimate f from the fitted S0 and the measured S0 and finally estimate D* while fixing D and f. This fit
    is done on an array.
    :param bvalues: 1D Array with the b-values
    :param TEs: 1D Array with TEs
    :param dw_data: 2D Array with diffusion-weighted signal in different voxels at different b-values
    :param njobs: Integer determining the number of parallel processes; default = 4
    :param bounds: 2D Array with fit bounds (Fp0, Dt, Fp, Dp, T2p, T2t). Default: ([0, 0, 0, 0, 0, 0], [2.5, 0.003, 1, 0.05, 0.15, 0.12])
    :param cutoff: cutoff value for determining which data is taken along in fitting D # TODO: what should this be in the case of T2-IVIM
    :return Dt: 1D Array with D in each voxel
    :return Fp: 1D Array with f in each voxel
    :return Dp: 1D Array with Dp in each voxel
    :return T2p: 1D Array with T2p in each voxel
    :return T2t: 1D Array with T2t in each voxel
    :return S0: 1D Array with S0 in each voxel # TODO: we have fp0 and fp instead for T2-IVIM
    """
    # first we normalise the signal to S0
    S0 = np.max(dw_data, axis=1)
    dw_data = dw_data / S0[:, None]
    # here we try parallel computing, but if fails, go back to computing one single core.
    single = False
    if njobs > 2:
        try:
            # define the parallel function
            def parfun(i):
                return fit_segmented_T2_ivim(bvalues, TEs, dw_data[i, :], bounds=bounds, cutoff=cutoff)

            output = Parallel(n_jobs=njobs)(delayed(parfun)(i) for i in tqdm(range(len(dw_data)), position=0, leave=True))
            Dt, Fp, Dp, T2p, T2t = np.transpose(output)
        except:
            # if fails, retry using single core
            single = True
    else:
    ...
    if single:
        ...
        Dt = np.zeros(len(dw_data))
        Fp = np.zeros(len(dw_data))
        T2p = np.zeros(len(dw_data))
        T2t = np.zeros(len(dw_data))
        for i in tqdm(range(len(dw_data)), position=0, leave=True):
            # fill arrays with fit results on a per voxel base:
            Dt[i], Fp[i], Dp[i], T2p[i], T2t[i] = fit_segmented_T2_ivim(bvalues, TEs, dw_data[i, :], bounds=bounds, cutoff=cutoff)
    return [Dt, Fp, Dp, T2p, T2t, S0]
...
# TODO: double check with tri-exp
def fit_segmented_T2_ivim(bvalues, TEs, dw_data, bounds=([0, 0, 0, 0, 0, 0], [2.5, 0.003, 1, 0.05, 0.15, 0.12]), cutoff=75):
    """
    This is an implementation of the segmented fit, in which we first estimate D using a curve fit to b-values>cutoff;
    then estimate f from the fitted S0 and the measured S0 and finally estimate D* while fixing D and f.
    :param bvalues: Array with the b-values
    :param TEs: Array with TEs
    :param dw_data: Array with diffusion-weighted signal at different b-values
    :param bounds: 2D Array with fit bounds (Fp0, Dt, Fp, Dp, T2p, T2t). Default: ([0, 0, 0, 0, 0, 0], [2.5, 0.003, 1, 0.05, 0.15, 0.12])
    :param cutoff: cutoff value for determining which data is taken along in fitting D
    :return Dt: Fitted D
    :return Fp: Fitted f
    :return Dp: Fitted Dp
    :return T2p: Fitted T2p
    :return T2t: Fitted T2t
    :return S0: Fitted S0
    """
    try:
        # determine high b-values and data for D
        high_b = bvalues[bvalues >= cutoff]
        high_TE = TEs[bvalues >= cutoff]
        high_dw_data = dw_data[bvalues >= cutoff]

        # correct the bounds. Note that S0 bounds determine the max and min of f
        bounds1 = ([0, 0, 0], [0.003*1000, 0.12*10, 1*10]) # TODO: not sure whether correct
        
        
        # fit for Fp, D, and T2t
        params, _ = curve_fit(lambda x_data, Dt, T2t, int: int * np.exp(-x_data[0] * (Dt / 1000)) * np.exp(-x_data[1]/(T2t / 10)), high_b, high_dw_data,
                              p0=(1, 1, 1),
                              bounds=bounds1)
        Dt, T2t, Fp = params[0] / 1000, params[1] / 10, 1 - (params[2] / 10)

        # remove the diffusion part to only keep the pseudo-diffusion
        dw_data_remaining = dw_data - (1 - Fp) * np.exp(-bvalues * Dt) * np.exp(-TEs/T2t)
        bounds2 = ([0, 0], [0.05, 0.15])

        # fit for D* and T2p
        params, _ = curve_fit(lambda x_data, Dp, T2p: Fp * np.exp(-x_data[0] * Dp) * np.exp(-x_data[1] / T2p), bvalues, dw_data_remaining, p0=(0.1, 0.1), bounds=bounds2)
        Dp, T2p = params[0], params[1]
        return Dt, Fp, Dp, T2p, T2t
    except:
        # if fit fails, return zeros
        print('segmented fit failed')
        return 0., 0., 0., 0., 0.
...
def fit_least_squares_array_T2_ivim(bvalues, TEs, dw_data, S0_output=True, fitS0=True, njobs=4,
                            bounds=([0, 0, 0, 0, 0.02, 0.04], [0.003, 1, 0.05, 1, 0.15, 0.12])): # TODO: include fp0?
    """
    This is an implementation of a tri-exponential fit. It is fitted in array form.
    :param bvalues: 1D Array with the b-values
    :param dw_data: 2D Array with diffusion-weighted signal in different voxels at different b-values
    ...
    :param fix_S0: Boolean determining whether to fix S0 to 1; default = False
    :param njobs: Integer determining the number of parallel processes; default = 4
    :param bounds: Array with fit bounds ([fp0min, Dtmin, Fpmin, Dpmin, T2pmin, T2tmin],[fp0max, Dtmax, Fpmax, Dpmax, T2pmax, T2tmax]). Default: ([0, 0, 0, 0.005, 0.02, 0.04], [2.5, 0.005, 1, 0.06, 0.02, 0.04])
    :return S0: optional 1D Array with S0 in each voxel
    :return Dt: 1D Array with D in each voxel
    ...
    if njobs > 1:
        try:
            # defining parallel function
            def parfun(i):
                return fit_least_squares_T2_ivim(bvalues, TEs, dw_data[i, :], S0_output=S0_output, fitS0=fitS0, bounds=bounds)

            output = Parallel(n_jobs=njobs)(delayed(parfun)(i) for i in tqdm(range(len(dw_data)), position=0, leave=True))
            Fp0, Dt, Fp, Dp, T2p, T2t = np.transpose(output)
        except:
            single = True
    else:
        single = True
    if single:
        ...
        # running in a single loop and filling arrays
        for i in tqdm(range(2), position=0, leave=True): # changed range(len(dw_data)) to 3 for testing
            Fp0[i], Dt[i], Fp[i], Dp[i], T2p[i], T2t[i] = fit_least_squares_T2_ivim(bvalues, TEs, dw_data[i, :], S0_output=S0_output, fitS0=fitS0,
                                                           bounds=bounds)

            xdata = np.hstack((bvalues, TEs))
            datasim = T2_ivim(xdata, Fp0[i], Dt[i], Fp[i], Dp[i], T2p[i], T2t[i])

            inds = np.lexsort((TEs, bvalues))

            dw_data_i = dw_data[i, inds]
            datasim = datasim[inds]
            ...
            for j, te_value in enumerate([0.076, 0.06, 0.11]):
                te_index = np.where(TEs == te_value)[0]

                print(f"te index: {te_index}")

                print(f"shape datasim[te_index]: {datasim[te_index].shape}")
                print(f"shape dw_data_i[te_index]: {dw_data_i[te_index].shape}")

                plt.plot(bvalues[te_index], dw_data_i[te_index], 'o', color=colors[j], label='data')
                plt.plot(bvalues[te_index], datasim[te_index], color=colors[j],
    ...
    if S0_output:
        return [Fp0+Fp, Dt, Fp/(Fp0+Fp), Dp, T2p, T2t]
    else:
        return [Dt, Fp/(Fp0+Fp), Dp, T2p, T2t]
...
def fit_least_squares_T2_ivim(bvalues, TEs, dw_data, S0_output=False, fitS0=True,
                      bounds=([0, 0, 0, 0, 0, 0], [2.5, 0.005, 1, 0.06, 0.2, 0.2])):
    """
    This is an implementation of the tri-exponential fit. It fits a single curve
    :param bvalues: 1D Array with the b-values
    :param TEs: 1D Array with TEs
    ...
    :param S0_output: Boolean determining whether to output (often a dummy) variable S0; default = True
    :param fix_S0: Boolean determining whether to fix S0 to 1; default = False
    :param bounds: Array with fit bounds Fp0, Dt, Fp, Dp, T2p, T2t. Default: ([0, 0, 0, 0, 0, 0], [2.5, 0.003, 1, 0.05, 0.15, 0.12])
    :return Fp0: optional 1D Array with f0 in each voxel
    :return Dt: 1D Array with D in each voxel
    :return Fp1: 1D Array with fp1 in each voxel
    :return Dp1: 1D Array with Dp1 in each voxel
    :return Fp2: 1D Array with the fraction of signal for Dp2 in each voxel
    :return Dp2: 1D Array with Dp2 in each voxel
    """

    # Flatten data to use in curve_fit
    xdata = np.hstack((bvalues, TEs))
    ...
    if not fitS0:
        ...
        bounds = ([bounds[0][1] * 1000, bounds[0][2] * 10, bounds[0][3] * 100, bounds[0][4] * 10, bounds[0][5] * 10],
                  [bounds[1][1] * 1000, bounds[1][2] * 10, bounds[1][3] * 100, bounds[1][4] * 10, bounds[1][5] * 10])
        params, _ = curve_fit(T2_ivimN_noS0, xdata, dw_data, p0=[1.5, 1, 3, 1, 1.5], bounds=bounds) # TODO: check curve fit function and how to incorporate both b and TE
        Fp0 = 1 - params[1] / 10
        Dt, Fp, Dp, T2p, T2t = params[0] / 1000, params[1] / 10, params[2] / 100, params[3] / 10, params[4] / 10
    else:
        # bounds are rescaled such that each parameter changes at roughly the same rate to help fitting.
        bounds = ([bounds[0][0] * 10, bounds[0][1] * 1000, bounds[0][2] * 10, bounds[0][3] * 100, bounds[0][4] * 10, bounds[0][5] * 10],
                  [bounds[1][0] * 10, bounds[1][1] * 1000, bounds[1][2] * 10, bounds[1][3] * 100, bounds[1][4] * 10, bounds[1][5] * 10])
        print(f'boundaries: {bounds}')
        params, _ = curve_fit(T2_ivimN, xdata, dw_data, p0=[8, 1.0, 1, 3, 1, 1], bounds=bounds)
        print(f"params: {params}")
        Fp0 = params[0]/10
        Dt, Fp, Dp, T2p, T2t = params[1] / 1000, params[2] / 10, params[3] / 100, params[4] / 10, params[5] / 10
    ...
    if S0_output:
        #Dt, Fp, Dp, Fp2, Dp2 = order_tri(Dt, Fp, Dp, Fp2, Dp2)
        return Fp0, Dt, Fp, Dp, T2p, T2t
    else:
        return Dt, Fp, Dp, T2p, T2t
    # except:
    #     # if fit fails, then do a segmented fit instead
    #     # print('lsq fit failed')
    #     if S0_output:
    #         return 0, 0, 0, 0, 0, 0
    #     else:
    #         return 0, 0, 0, 0, 0
