"""
October 2023 by K.E. Spaans

Built on code by Oliver Gurney-Champion
oliver.gurney.champion@gmail.com / o.j.gurney-champion@amsterdamumc.nl
https://www.github.com/ochampion

and Sebastiano Barbieri: https://github.com/sebbarb/deep_ivim
...
import os
import IVIMNET_model.IVIMNET.fitting_algorithms as fit
...
class Net(nn.Module):
    def __init__(self, bvalues, TEs, net_pars, train_pars): # KS: TE as extra parameter
        """
        this defines the Net class which is the network we want to train.
        :param bvalues: a 1D array with the b-values
        :param TEs: a 1D array with echo times
        ...
        :param train_pars: object with training design options
        """
        super(Net, self).__init__()
        self.counter = 0
        self.bvalues = bvalues
        self.TEs = TEs # KS: added
        ...
        if self.net_pars.width_first_layer is 0:
            self.net_pars.width_first_layer = len(bvalues)
        # define number of parameters being estimated
        if self.net_pars.tri_exp:
            self.est_pars = 5 # KS: should be 5 for T2-IVIM
        elif self.net_pars.T2: # KS: added
            self.est_pars = 5
        else:
            self.est_pars = 3
        ...
        if self.net_pars.parallel != 'single':
            ...
            if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                self.fc_layers4 = nn.ModuleList()
                self.fc_layers5 = nn.ModuleList()
            elif self.net_pars.T2: # KS: added
                if self.net_pars.fix_T2p:
                    self.fc_layers4 = nn.ModuleList()
                else:
                    self.fc_layers4 = nn.ModuleList()
                    self.fc_layers5 = nn.ModuleList()
        ...
        for i in range(self.net_pars.depth):
            input_width = self.net_pars.width_first_layer if i == 0 and self.net_pars.width_first_layer != 0 else self.net_pars.width

            # extend with a fully-connected linear layer
            self.fc_layers0.extend([nn.Linear(input_width, self.net_pars.width)])
            if self.net_pars.parallel != 'single':
                self.fc_layers1.extend([nn.Linear(input_width, self.net_pars.width)])
                self.fc_layers2.extend([nn.Linear(input_width, self.net_pars.width)])
                self.fc_layers3.extend([nn.Linear(input_width, self.net_pars.width)])
                if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                    self.fc_layers4.extend([nn.Linear(input_width, self.net_pars.width)])
                    self.fc_layers5.extend([nn.Linear(input_width, self.net_pars.width)])
                elif self.net_pars.T2: # KS: added
                    if self.net_pars.fix_T2p:
                        self.fc_layers4.extend([nn.Linear(input_width, self.net_pars.width)])
                    else:
    ...
    def forward(self, X):
        ...
        if self.net_pars.con == 'sigmoid':
            ...
            if self.net_pars.T2: # KS: added
                if self.net_pars.fix_T2p:
                    ...
                    T2tmin = self.net_pars.cons_min[4]
                    T2tmax = self.net_pars.cons_max[4]
                else:
                    Dmin = self.net_pars.cons_min[0]
                    Dmax = self.net_pars.cons_max[0]
                    fmin = self.net_pars.cons_min[1]
                    ...
                    f0min = self.net_pars.cons_min[3]
                    f0max = self.net_pars.cons_max[3]
                    T2pmin = self.net_pars.cons_min[4]
                    T2pmax = self.net_pars.cons_max[4]
                    T2tmin = self.net_pars.cons_min[5]
                    T2tmax = self.net_pars.cons_max[5]
            ...
            if self.net_pars.con == 'abs': # skipped for fixing T2p
        ...
        if self.net_pars.con == 'sigmoid':
            ...
            if self.net_pars.parallel == 'parallel' or self.net_pars.parallel == 'semi_parallel':
                ...
                if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                    ...
                    if self.net_pars.fix_T2p:
                        T2t = T2tmin + torch.sigmoid(params3[:, 0].unsqueeze(1)) * (T2tmax - T2tmin)
                        if self.net_pars.fitS0:
                            f0 = f0min + torch.sigmoid(params4[:, 0].unsqueeze(1)) * (f0max - f0min)
                    else:
                        T2p = T2pmin + torch.sigmoid(params3[:, 0].unsqueeze(1)) * (T2pmax - T2pmin)
                        T2t = T2tmin + torch.sigmoid(params4[:, 0].unsqueeze(1)) * (T2tmax - T2tmin)
                        if self.net_pars.fitS0:
                            f0 = f0min + torch.sigmoid(params5[:, 0].unsqueeze(1)) * (f0max - f0min)
                elif self.net_pars.fitS0:
                ...
                if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM, need to figure out the constraints for T2p and T2t
                ...
                if self.net_pars.T2: # KS: added
                    if self.net_pars.fix_T2p:
                        T2t = T2tmin + torch.sigmoid(params0[:, 3].unsqueeze(1)) * (T2tmax - T2tmin)
                        if self.net_pars.fitS0:
                            f0 = f0min + torch.sigmoid(params0[:, 4].unsqueeze(1)) * (
                                    f0max - f0min)
                    else:
                        T2p = T2pmin + torch.sigmoid(params0[:, 3].unsqueeze(1)) * (T2pmax - T2pmin)
                        T2t = T2tmin + torch.sigmoid(params0[:, 4].unsqueeze(1)) * (T2tmax - T2tmin)
                        if self.net_pars.fitS0:
                            f0 = f0min + torch.sigmoid(params0[:, 5].unsqueeze(1)) * (
                                    f0max - f0min)
                elif self.net_pars.fitS0:
            ...
            if self.net_pars.parallel == 'parallel' or self.net_pars.parallel == 'semi_parallel':
                ...
                if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                    ...
                    T2p = params3[:, 0].unsqueeze(1)
                    T2t = params4[:, 0].unsqueeze(1)
                    if self.net_pars.fitS0:
                        f0 = params5[:, 0].unsqueeze(1)
                elif self.net_pars.fitS0:
                ...
                if self.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                    ...
                    T2p = params0[:, 3].unsqueeze(1)
                    T2t = params0[:, 4].unsqueeze(1)
                    if self.net_pars.fitS0:
                        f0 = params0[:, 5].unsqueeze(1)
                elif self.net_pars.fitS0:
        ...
        # the loss function.
        if self.net_pars.tri_exp:
            if self.net_pars.fitS0: # KS: adjust this to fit the formula for the T2-IVIM model
        ...
        elif self.net_pars.T2: # KS: corrected formula for T2-IVIM
            if self.net_pars.fix_T2p:
                T2p = torch.full((len(Dp), 1), self.net_pars.T2p_fixed_val).to(self.train_pars.device)

                if self.net_pars.fitS0:
                    if self.net_pars.segmented_fit:
                        X_temp.append((Fp * torch.exp(-self.bvalues * Dp) + f0 * torch.exp(-self.bvalues * Dt)))
                    else:
                        X_temp.append((Fp * torch.exp(-self.TEs / T2p) * torch.exp(-self.bvalues * Dp) +
                                       f0 * torch.exp(-self.TEs / T2t) * torch.exp(-self.bvalues * Dt)))
                else:
                    X_temp.append((Fp * torch.exp(-self.TEs / T2p) * torch.exp(-self.bvalues * Dp) +
                                   (1 - Fp) * torch.exp(-self.TEs / T2t) * torch.exp(-self.bvalues * Dt)))
        ...
        if self.net_pars.tri_exp: # KS: adjust this to fit the formula for the T2-IVIM model
            if self.net_pars.fitS0:
            ...
            if self.net_pars.fix_T2p:
                if self.net_pars.fitS0:
                    return X, Dt, Fp/(f0+Fp), Dp, f0+Fp, T2t # T2p is fixed
                else:
                    return X, Dt, Fp, Dp, torch.ones(len(Dt)), T2t # T2p is fixed
            else:
                if self.net_pars.fitS0:
                    return X, Dt, Fp/(f0+Fp), Dp, f0+Fp, T2p, T2t
                else:
                    return X, Dt, Fp, Dp, torch.ones(len(Dt)), T2p, T2t
        else:
            if self.net_pars.fitS0:
    ...
    def freeze_T2_params(self, net):
        """
        Function to freeze T2p and T2t while training on data subset.
        Note: only implemented for parallel network so far.
        """
        if self.net_pars.fix_T2p:
            ...
            for layer in [self.encoder3, self.encoder4]:
                for param in layer.parameters():
                    param.requires_grad = False

    def unfreeze_T2_params(self, net):
        """
        Function to unfreeze T2p and T2t after training on data subset.
        Note: only implemented for parallel network so far.
        """
        if self.net_pars.fix_T2p:
            for layer in [self.encoder3]:
    ...
    def freeze_other_params(self, net):
        """
        Function to freeze D, f, and D* while training.
        Note: only implemented for parallel network so far.
        """

        for layer in [self.encoder0, self.encoder1, self.encoder2]:
            for param in layer.parameters():
                param.requires_grad = False

    def unfreeze_other_params(self, net):
        """
        Function to unfreeze D, f, and D* after training.
        Note: only implemented for parallel network so far.
        """
...
def learn_IVIM(X_train, bvalues, TEs, arg, net=None): # KS: added TE as an extra parameter here
    """
    This program builds a IVIMNET_model network and trains it.
    :param X_train: 2D array of IVIM data we use for training. First axis are the voxels and second axis are the b-values
    :param bvalues: a 1D array with the b-values
    :param TEs: a 1D array with the echo times
    ...
    X_train = normalise(X_train, bvalues, TEs, arg, min(bvalues))
    ...
    if net is None:
        ...
        TEs = torch.FloatTensor(TEs[:]).to(arg.train_pars.device)
        net = Net(bvalues, TEs, arg.net_pars, arg.train_pars).to(arg.train_pars.device)
    else:
    ...
    num_epochs = 1000 # regular training loop (no freezing)

    ## Train
    for stage in range(arg.train_pars.stages):
        num_bad_epochs = 0
        print(f'Stage {stage}:')
        if arg.train_pars.freeze_T2:
            if stage == 0:
                print(f"freeze T2-predictions")
                net.freeze_T2_params(net)
                print(f"segmented fit = {net.net_pars.segmented_fit}")

            elif stage == 1:
                best = 1e16
                print(f"unfreeze T2-predictions")
                net.unfreeze_T2_params(net)

                print(f"freeze D, f, and D* predictions")
                net.freeze_other_params(net)

                net.net_pars.segmented_fit = False
                print(f"segmented fit = {net.net_pars.segmented_fit}")

            elif stage == 2:
                print(f"unfreeze all")
                net.unfreeze_other_params(net)
        ...
        for epoch in range(num_epochs):
            ...
            for i, X_batch in enumerate(tqdm(trainloader, position=0, leave=True, total=totalit), 0):
                ...
                elif arg.net_pars.T2:
                    if arg.net_pars.fix_T2p:
                        X_pred, D_pred, Fp_pred, Dp_pred, S0_pred, T2t_pred = net(X_batch)
                    else:
                        X_pred, D_pred, Fp_pred, Dp_pred, S0_pred, T2p_pred, T2t_pred = net(X_batch)
                else:
                    X_pred, D_pred, Fp_pred, Dp_pred, _ = net(X_batch)
                ...
                if net.net_pars.segmented_fit:
                    # select data with short TE only - for stage 0 in segmented fit
                    X_pred = X_pred[:, TEs < 0.08]
                    X_batch = X_batch[:, TEs < 0.08]
            ...
            for i, X_batch in enumerate(tqdm(inferloader, position=0, leave=True), 0):
                ...
                if arg.net_pars.tri_exp:
                    ...
                    if arg.net_pars.fix_T2p:
                        X_pred, _, _, _, _, _ = net(X_batch)
                    else:
                        X_pred, _, _, _, _, _, _ = net(X_batch)
                else:
                    X_pred, Dtt, Fpt, Dpt, S0t = net(
                        X_batch)  # removing nans and too high/low predictions to prevent overshooting
                ...
                if i > 0:
                    ...
                    sns.heatmap(Dtt.cpu().detach().numpy())
                    plt.show()
                    sns.heatmap(Fpt.cpu().detach().numpy())
                    plt.show()
                    sns.heatmap(Dpt.cpu().detach().numpy())
                    plt.show()
                """
            ...
            if arg.train_pars.scheduler:
                print('Current learning rate before scheduler step:', optimizer.param_groups[0]['lr'])
                scheduler.step(running_loss_val)
                print('Current learning rate after scheduler step:', optimizer.param_groups[0]['lr'])
            ...
            if epoch > 0 and not net.net_pars.segmented_fit:
                # plot progress and intermediate results (if enabled)
                plot_progress_multiple_TEs(X_batch.cpu(), X_pred.cpu(), bvalues.cpu(), TEs.cpu(), loss_train,
                                           loss_val,
                                           arg)
...
def parameter_distribution_plot(D_pred, Dp_pred, Fp_pred, T2t_pred, cpu=True, text=None, T2p_pred=None):
    ...
    if cpu:
        ...
        D_pred_plot = D_pred
    plt.hist(D_pred_plot[~np.isnan(D_pred_plot)], bins=20, alpha=0.5, color='green', label='D_pred')
    ...
    plt.hist(Dp_pred_plot[~np.isnan(Dp_pred_plot)], bins=20, alpha=0.5, color='purple', label='Dp_pred')
    ...
    if cpu:
        ...
        if T2p_pred is None:
            ...
            T2t_pred_plot = T2t_pred
    if T2p_pred is None:
        plt.hist(T2t_pred_plot[~np.isnan(T2t_pred_plot)], bins=20, alpha=0.5, color='red', label='T2t_pred')
    else:
        plt.hist(T2p_pred_plot[~np.isnan(T2p_pred_plot)], bins=20, alpha=0.5, color='orange', label='T2p_pred')
        plt.hist(T2t_pred_plot[~np.isnan(T2t_pred_plot)], bins=20, alpha=0.5, color='red', label='T2t_pred')
    ...
    if cpu:
        ...
        Fp_pred_plt = Fp_pred
    plt.hist(Fp_pred_plt[~np.isnan(Fp_pred_plt)], alpha=0.5, color='blue', label='Fp_pred (Fp/(f0+Fp))')
    plt.xlim(-0.005, 1)
...
def load_optimizer(net, arg):
    if arg.train_pars.optim == 'adam':
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=arg.train_pars.lr, weight_decay=1e-3)#, amsgrad=True)
...
def predict_IVIM(data, bvalues, TEs, net, arg):
    """
    ...
    :param bvalues: a 1D array with the b-values
    :param TEs: a 1D array with echo times
    :param net: the trained IVIMNET_model network
    :param arg: an object with network design options, as explained in the publication check hyperparameters.py for
    options
    ...
    ## normalise the signal to b=0 and remove data with nans
    data = normalise(data, bvalues, TEs, arg)
    ...
    Fp = np.array([])
    S0 = np.array([])
    if arg.net_pars.tri_exp: # KS: should be similar for T2-IVIM
        Fp2 = np.array([])
        Dp2 = np.array([])
    elif arg.net_pars.T2: # KS: added
        if arg.net_pars.fix_T2p:
            T2t = np.array([])
        else:
            T2p = np.array([])
    ...
    with torch.no_grad():
        for i, X_batch in enumerate(tqdm(inferloader, position=0, leave=True), 0):
            ...
            if arg.net_pars.tri_exp: # KS: should be similar for T2-IVIM
                ...
                if arg.net_pars.fix_T2p:
                    _, Dtt, Fpt, Dpt, S0t, T2tt = net(X_batch)
                    T2t = np.append(T2t, (T2tt.cpu()).numpy())
                else:
                    _, Dtt, Fpt, Dpt, S0t, T2pt, T2tt = net(X_batch)
                    T2p = np.append(T2p, (T2pt.cpu()).numpy())
                    T2t = np.append(T2t, (T2tt.cpu()).numpy())
            else:
    ...
    if np.mean(Dp) < np.mean(Dt):
        ...
        Dt = copy.deepcopy(Dp)
        Dp = copy.deepcopy(Dp22)
        if arg.net_pars.tri_exp: # KS: this is only for the tri_exp model specifically I think
            Fp = 1 - Fp - Fp2
        else:
    ...
    if arg.net_pars.T2: # KS: added
        if arg.net_pars.fix_T2p:
            T2ttrue = np.zeros(lend)
            T2ttrue[sels] = T2t
            return [Dttrue, Fptrue, Dptrue, T2ttrue, S0true]
        else:
            T2ptrue = np.zeros(lend)
            T2ttrue = np.zeros(lend)
            T2ptrue[sels] = T2p
            T2ttrue[sels] = T2t
            return [Dttrue, Fptrue, Dptrue, T2ptrue, T2ttrue, S0true]
    else:
        return [Dttrue, Fptrue, Dptrue, S0true]


def normalise(X_train, bvalues, TEs, arg, bref=0):
    try:
        ## normalise the signal to b=0 and remove data with nans
        if arg.norm_data_full:
            if arg.net_pars.T2:
                S0 = np.max(X_train, axis=1).astype('<f')
            else:
        ...
        num_zeros = np.sum(np.where(S0[:, None] == 0, 1, 0))
        print(f"num_zeroes: {num_zeros}")

        S0_modified = np.where(S0[:, None] == 0, 1e-5, S0[:, None])

        X_train = X_train / S0_modified
...
def TE_index(TEs, te_values=[0.076, 0.11, 0.06]):
    """
    Takes as input different TE values, the TEs to be indexed and returns a list of indices for the specific TEs.
    """
    indices = []

    for i, te_value in enumerate(te_values):
        te_index = np.where(TEs == te_value)[0]
        indices.append(te_index)

    return indices, te_values
...
def plot_progress(X_batch, X_pred, bvalues, loss_train, loss_val, arg): # KS: need to account for different TEs
    # this program plots the progress of the training. It will plot the loss and validation loss, as well as 4 IVIM curve
...
def save_final_plots(X_batch, X_pred, bvalues, TEs, loss_train, loss_val, arg, te_values=[0.076, 0.11, 0.06]):
    ...
    # Plotting IVIM curves for 3 different TEs
    ...
    for j in range(4):
        ...
        for i, te_value in enumerate(te_values):
            ...
            plt.plot(bvalues[te_index], X_batch.data[j, te_index], 'o', color=colors[i], label='data')
            plt.plot(bvalues[te_index], X_pred.data[j, te_index], color=colors[i],
                     label=f'estimate TE={te_value * 1000}')

        plt.ylim(min(X_batch.data[0]) - 0.3, 1.2 * max(X_batch[0]))
...
def plot_progress_multiple_TEs(X_batch, X_pred, bvalues, TEs, loss_train, loss_val, arg, te_values=[0.076, 0.11, 0.06]):
    inds = np.lexsort((TEs, bvalues))
    ...
    if arg.fig:
        ...
        for j in range(4):
            ...
            for i, te_value in enumerate(te_values):
                ...
                plt.plot(bvalues[te_index], X_batch.data[j, te_index], 'o', color=colors[i], label=f'TE={te_value*1000}')
                # plt.plot(bvalues[te_index], X_pred.data[j, te_index], color=colors[i], label=f'estimate TE={te_value*1000}')
...
def make_data_complete(dw_data,bvalues,fraction_threshold=0.2): # KS: haven't seen this function called anywhere so skip for now
    """
...
def checkarg_train_pars(arg):
    ...
    if not hasattr(arg,'split'):
        ...
        arg.split = 0.9  # split of IVIM_get_parameter_maps.py and validation data
    ...
    if not hasattr(arg, 'plateau_size'):
        arg.plateau_size = 5
        warnings.warn(f'arg.train.plateau_size not defined. Using default of {arg.plateau_size}')
    return arg
...
class train_pars:
    def __init__(self,nets):
        ...
        if nets == 'optim':
            ...
            self.lr = 0.00003 # this is the learning rate.
        self.patience= 5 # this is the number of epochs without improvement that the network waits untill determining it found its optimum
...
class net_pars:
    def __init__(self,nets):
        ...
        if self.tri_exp: # KS: should be similar for T2-IVIM
            self.cons_min = [0., 0.0003, 0.0, 0.003, 0.0, 0.08] # F0', D0, F1', D1, F2', D2
            self.cons_max = [2.5, 0.003, 1, 0.08, 1, 5]  # F0', D0, F1', D1, F2', D2
        elif self.T2: # KS: added -- note that in thesis of Betsch the max value of Fp was set to 0.3 instead of 0.1
            print('we here')
            self.cons_min = [0, 0, 0, 0, 0.02, 0.04]  # Dt, Fp, Ds, S0, T2p, T2t
            self.cons_max = [0.003, 1, 0.05, 1, 0.15, 0.12]  # Dt, Fp, Ds, S0, T2p, T2t
            print('after establishing cons_max and cons_min')
            print(self.cons_min, self.cons_max)
        else:
...
class lsqfit:
    def __init__(self):
        ...
        self.model = 'bi-exp' #seg, bayes or lsq -- KS: LSQ should work with T2-IVIM as well... 
