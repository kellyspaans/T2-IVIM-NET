        ...
        if not arg.load_lsq:
            ...
            if arg.method == 'seg':
                if arg.model == 'bi-exp':
                    ...
                    paramslsq = fit_segmented_array_T2_ivim(bvalues, dw_data, njobs=arg.jobs, bounds=arg.bounds)
                    # save results if parameter savename is given
                    if savename is not None:
                        np.savez(savename, paramslsq=paramslsq)
                ...
                if arg.model == 'bi-exp':
                    ...
                    paramslsq = fit_least_squares_array_T2_ivim(bvalues, dw_data, S0_output=True, fitS0=arg.fitS0,
                                                                njobs=arg.jobs,
                                                                bounds=arg.bounds)
                    # save results if parameter savename is given
                    if savename is not None:
                ...
                if arg.model == 'bi-exp':
                    ...
                    raise Exception('the choice tri-exp bayes not implemented')
                elif (arg.model == 'T2'):
...
def goodness_of_fit(bvalues, Dt, Fp, Dp, S0, dw_data, Fp2 = None, Dp2 = None, T2p=None, T2t=None):
    """
    ...
    try:
        if Fp2 is None and T2p is None:
        ...
        elif T2p is not None: # TODO: check whether values are correct, why are only some variables multiplied by S0?
            datasim = T2_IVIM(np.tile(np.expand_dims(bvalues, axis=0), (len(Dt), 1)),
                           ...
                           np.tile(np.expand_dims(Dp, axis=1), (1, len(bvalues))),
                           np.tile(np.expand_dims(T2p*S0, axis=1), (1, len(bvalues))),
        ...
        if Fp2 is None and T2p is None:
            ...
            datasim = T2_IVIM(bvalues, S0 * (1 - Fp), Dt, Fp * S0, Dp, T2p * S0, T2t) # TODO: ?
        else:
            datasim = tri_exp(bvalues, S0 * (1 - Fp - Fp2), Dt, Fp * S0, Dp, Fp2 * S0, Dp2) # TODO: ?
...
def MSE(bvalues, Dt, Fp, Dp, S0, dw_data): # TODO: not implemented for T2-IVIM yet
    """
...
def T2_ivimN(bvalues, TEs, Dt, Fp, Dp, T2p, T2t, S0):
    # IVIM function in which we try to have equal variance in the different IVIM parameters; equal variance helps with certain fitting algorithms
    return S0 * T2_ivimN_noS0(bvalues, TEs, Dt, Fp, Dp, T2p, T2t)


def T2_ivimN_noS0(bvalues, TEs, Dt, Fp, Dp, T2p, T2t):
    # IVIM function in which we try to have equal variance in the different IVIM parameters and S0=1
    return ((Fp / 10) * np.exp(-bvalues * (Dp / 10)) * np.exp(-TEs/(T2p / 10)) + (1 - (Fp / 10)) * np.exp(-bvalues * (Dt / 1000)) * np.exp(-TEs/(T2t / 10))) # TODO: check the divisions for T2p and T2t


def T2_ivim(bvalues, TEs, Dt, Fp, Dp, T2p, T2t, S0): # TODO: keep S0? Not included in tri_exp.
    # regular IVIM function
    return (S0 * (Fp * np.exp(-bvalues * Dp) * np.exp(-TEs / T2p) + (1 - Fp) * np.exp(-bvalues * Dt) * np.exp(-TEs / T2t)))
