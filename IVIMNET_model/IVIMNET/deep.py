...
class Net(nn.Module):
    def __init__(self, bvalues, net_pars): 
        ...
        if self.net_pars.tri_exp:
            self.est_pars = 5 # KS: should be 5 for T2-IVIM as well
        else:
            self.est_pars = 3
        if self.net_pars.fitS0:
            self.est_pars += 1 # KS: what is this?
        ...
        if self.net_pars.parallel == 'parallel':
            ...
            if self.net_pars.tri_exp: # KS: why are these layers deleted for semi-parallel networks?
                del self.fc_layers3 
                del self.fc_layers4
                if self.net_pars.fitS0:
                    del self.fc_layers5
    ...
    def forward(self, X):
        ...
        if self.net_pars.con == 'sigmoid':
            # define constraints
            if self.net_pars.tri_exp: # KS: for T2-IVIM model, constraints should be set for T2p and T2t, how can I gain insights in this?
...
def learn_IVIM(X_train, bvalues, arg, net=None):
    ...
    for epoch in range(1000):
        ...
        for i, X_batch in enumerate(tqdm(trainloader, position=0, leave=True, total=totalit), 0):
            ...
            if arg.net_pars.tri_exp: # KS: should be similar for T2-IVIM, parameters
                X_pred, _, _, Fp_pred, Dp_pred, _, _ = net(X_batch)
            else:
                X_pred, D_pred, Fp_pred, Dp_pred, _ = net(X_batch)            # removing nans and too high/low predictions to prevent overshooting
        ...
        for i, X_batch in enumerate(tqdm(inferloader, position=0, leave=True), 0):
            ...
            if arg.net_pars.tri_exp: # KS: what is the difference between tri_exp and bi_exp here?
                X_pred, _, _, _, _, _, _ = net(X_batch)
            else:

                X_pred, Dtt, Fpt, Dpt, S0t = net(X_batch)            # removing nans and too high/low predictions to prevent overshooting
...
def predict_IVIM(data, bvalues, net, arg):
    ...
    if arg.train_pars.use_cuda:
        torch.cuda.empty_cache()
    if arg.net_pars.tri_exp: # KS: don't really understand what is happening here exactly
...
def normalise(X_train,bvalues,arg, bref=0): # KS: is it necessary involve TE in this part? 
    try:
        ## normalise the signal to b=0 and remove data with nans
...
def checkarg_net_pars(arg):
    ...
    if not hasattr(arg, 'width'):
        ...
        arg.width = 0
    if not hasattr(arg, 'tri_exp'): # KS: implement this as well for T2-IVIM?
    ...
    if not hasattr(arg,'cons_max'):
        if arg.tri_exp: # KS: implement this as well for T2-IVIM?
...
class net_pars:
    def __init__(self,nets):
        ...
        self.tri_exp = False # KS: is this the default value?
