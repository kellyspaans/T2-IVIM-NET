"""
January 2024 by K.E. Spaans - data simulation extended for T2-IVIM
...
# import libraries
import numpy as np
from . import deep_T2 as deep
from . import fitting_algorithms_T2 as fit
import time
...
def sim(SNR, arg):
    ...
    randomly selecting a value of D, f and D* (and T2p/T2t for T2-IVIM) from within the predefined range. The script calculates the random,
    ...
    (Dp), T2p, T2t, whereas the colums give the mean input value, the random error and the systematic error
    ...
    elif arg.fit.model == 'T2':
        IVIM_signal_noisy, D, f, Dp, T2p, T2t = sim_signal(SNR, arg.sim.bvalues, arg.sim.TEs, sims=arg.sim.sims, Dmin=arg.sim.range[0][0],
                                                 Dmax=arg.sim.range[1][0], fmin=arg.sim.range[0][1],
                                                 fmax=arg.sim.range[1][1], Dsmin=arg.sim.range[0][2],
                                                 Dsmax=arg.sim.range[1][2], T2pmin=arg.sim.range[0][3],
                                                 T2pmax=arg.sim.range[1][3],T2tmin=arg.sim.range[0][4],
                                                 T2tmax=arg.sim.range[1][4],rician=arg.sim.rician, model='T2')
        T2p = T2p[:arg.sim.num_samples_eval]
        T2t  = T2t[:arg.sim.num_samples_eval]
        dims=6
    ...
    if not arg.train_pars.skip_net:
        ...
        for aa in range(arg.sim.repeats):
            ...
            # normalize the data
            IVIM_signal_noisy = IVIM_signal_noisy / IVIM_signal_noisy.max()

            # remove NaN
            res = [i for i, val in enumerate(IVIM_signal_noisy != IVIM_signal_noisy) if not val.any()]

            print(f"data: {IVIM_signal_noisy[res].shape}")
            print(f"bval: {arg.sim.bvalues.shape}")
            print(f"TEs: {arg.sim.TEs.shape}")

            IVIM_signal_noisy = IVIM_signal_noisy[res]

            net = deep.learn_IVIM(IVIM_signal_noisy, arg.sim.bvalues, arg.sim.TEs, arg)
            elapsed_time = time.time() - start_time
            print('\ntime elapsed for training: {}\n'.format(elapsed_time))
            start_time = time.time()
            # predict parameters
            if arg.sim.repeats > 1:
                paramsNN[aa] = deep.predict_IVIM(IVIM_signal_noisy[:arg.sim.num_samples_eval, :], arg.sim.bvalues, arg.sim.TEs, net,
                                                 arg)
            else:
                paramsNN = deep.predict_IVIM(IVIM_signal_noisy[:arg.sim.num_samples_eval, :], arg.sim.bvalues, arg.sim.TEs, net,
                                             arg)
            elapsed_time = time.time() - start_time
        ...
        if arg.sim.repeats > 1:
            ...
            for aa in range(arg.sim.repeats):
                ...
                if arg.fit.model == 'bi-exp':
                    ...
                    matNN[aa] = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsNN[aa], np.squeeze(T2p), np.squeeze(T2t))
                else:
            ...
            if arg.fit.model == 'bi-exp':
                ...
                stability = stability[[0, 1, 2, 3 ,4]] / [np.mean(D), np.mean(f), np.mean(Dp), np.mean(T2p), np.mean(T2t)]
            else:
            ...
            if arg.fit.model == 'bi-exp':
                ...
                matNN = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsNN,np.squeeze(T2p),np.squeeze(T2t))
            else:
    ...
    if arg.fit.do_fit:
        ...
        paramsf = fit.fit_dats(arg.sim.bvalues, IVIM_signal_noisy[:arg.sim.num_samples_eval, :], arg.fit) # TODO
        elapsed_time = time.time() - start_time
        ...
        if arg.fit.model == 'bi-exp':
            ...
            matlsq = print_errors(np.squeeze(D), np.squeeze(f), np.squeeze(Dp), paramsf, np.squeeze(T2p), np.squeeze(T2t))
        else:
...
def sim_signal(SNR, bvalues, TEs, sims=100000, Dmin=0.5 / 1000, Dmax=5.0 / 1000, fmin=0.0, fmax=0.7, Dsmin=0.05, Dsmax=0.3, fp2min=0, fp2max=0.3, Ds2min=0.2, Ds2max=0.5, T2pmin=0.02,
               T2pmax=0.15, T2tmin=0.04, T2tmax=0.12, rician=False, state=123, model='T2'):
    """
    This simulates IVIM curves. Data is simulated by randomly selecting a value of D, f, D*, T2p, and T2t from within the
    predefined range.
    ...
    :param Dmax: maximal simulated f. Default = 0.5
    :param Dpmin: minimal simulated D*. Default = 0.05
    :param Dpmax: maximal simulated D*. Default = 0.2
    :param T2pmin: minimal simulated T2p. Default = 0.02
    :param T2pmax: maximal simulated T2p. Default = 0.15
    :param T2tmin: minimal simulated T2t. Default = 0.04
    :param T2tmax: maximal simulated T2t. Default = 0.12
    ...
    if model == 'tri-exp':
        ...
        test = rg.uniform(0, 1, (sims, 1))
        T2p = T2pmin + (test * (T2pmax - T2pmin))
        test = rg.uniform(0, 1, (sims, 1))
        T2t = T2tmin + (test * (T2tmax - T2tmin))
    ...
    for aa in range(len(D)):
        if model == 'bi-exp':
            ...
            data_sim[aa, :] = fit.T2_ivim(bvalues, TEs, 1-f[aa][0], D[aa][0], f[aa][0], Dp[aa][0], T2p[aa][0], T2t[aa][0])

        else:
            data_sim[aa, :] = fit.tri_exp(bvalues, 1-f[aa][0]-f2[aa][0], D[aa][0], f[aa][0], Dp[aa][0], f2[aa][0], Dp2[aa][0])

    print(f"datasim shape: {data_sim.shape}")
    ...
    if model == 'bi-exp':
        ...
        return data_sim, D, f, Dp, T2p, T2t
    else:
        return data_sim, D, f, Dp, f2, Dp2

def print_errors(D, f, Dp, params, f2=(), Dp2=(), T2p=(), T2t=()):
    ...
    if len(f2) == 0 and len(T2p) == 0:
        ...
        rmse_f = np.sqrt(np.nanmean(np.square(np.subtract(f, params[2]))))
        rmse_Dp = np.sqrt(np.nanmean(np.square(np.subtract(Dp, params[3]))))
        rmse_T2p = np.sqrt(np.nanmean(np.square(np.subtract(T2p, params[4]))))
        rmse_T2t = np.sqrt(np.nanmean(np.square(np.subtract(T2t, params[5]))))
        # initialise Spearman Rank matrix
        Spearman = np.zeros([5, 2])
        # calculate Spearman Rank correlation coefficient and p-value
        Spearman[0, 0], Spearman[0, 1] = scipy.spearmanr(params[0], params[2])  # DvDp
        ...
        normf_lsq = np.mean(f)
        normDp_lsq = np.mean(Dp)
        normT2p_lsq = np.mean(T2p)
        normT2t_lsq = np.mean(T2t)

        print(
            '\nresults from NN: columns show themean, the SD/mean, the systematic error/mean, the RMSE/mean and the Spearman coef [DvDp,Dvf,fvDp] \n'
            'the rows show D, f, D*, T2p, and T2t\n')
        print([normD_lsq, '  ', rmse_D / normD_lsq, ' ', Spearman[0, 0]])
        print([normf_lsq, '  ', rmse_f / normf_lsq, ' ', Spearman[1, 0]])
        print([normDp_lsq, '  ', rmse_Dp / normDp_lsq, ' ', Spearman[2, 0]])
        print([normT2p_lsq, '  ', rmse_T2p / normT2p_lsq, ' ', Spearman[3, 0]])
        print([normT2t_lsq, '  ', rmse_T2t / normT2t_lsq, ' ', Spearman[4, 0]])

        mats = [[normD_lsq, rmse_D / normD_lsq, Spearman[0, 0]],
                [normf_lsq, rmse_f / normf_lsq, Spearman[1, 0]],
                [normDp_lsq, rmse_Dp / normDp_lsq, Spearman[2, 0]],
                [normT2p_lsq, rmse_T2p / normT2p_lsq, Spearman[3, 0]],
                [normT2t_lsq, rmse_T2t / normT2t_lsq, Spearman[4, 0]]]

    return mats


def sim_signal_predict(arg, SNR): #TODO: not adjusted for T2-IVIM yet
    # init randomstate
...
def plot_example1(paramsNN, paramsf, Dt_truth, Fp_truth, Dp_truth, arg, SNR): #TODO: not adjusted for T2-IVIM yet
    # initialise figure
    sx, sy, sb = 100, 100, len(arg.sim.bvalues)
    if arg.fit.do_fit:
